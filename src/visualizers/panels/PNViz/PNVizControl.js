...
define([
    ...
    PNVizControl.prototype._initWidgetEventHandlers = function () {
    ...
    PNVizControl.prototype.selectedObjectChanged = function (nodeId) {
        var self = this;
    ...
    /* * * * * * * * Node Event Handling * * * * * * * */
    PNVizControl.prototype._eventCallback = function (events) {
        const self = this;
        console.log("*** PENDES EVENTS ****")
        console.log(events);
    ...
    PNVizControl.prototype._stateActiveObjectChanged = function (model, activeObjectId) {
    ...
    PNVizControl.prototype._initSM = function () {
        const self = this;
        ...
        elementIds.forEach(elementId => {
            ...
            if (node.isTypeOf(META['Place'])) {
                const place = {
                    ...
                    isTransition: node.isTypeOf(META['Transition']),
                    tokens: node.getAttribute('token')
                };
                var i = 1;
                elementIds.forEach(nextId => {
                    const nextNode = self._client.getNode(nextId);
                    if ((nextNode.isTypeOf(META['Inplaces']) || nextNode.isTypeOf(META['Outplaces'])) && nextNode.getPointerId('src') === elementId) {
                        place.next[nextNode.getAttribute('name') + i] = nextNode.getPointerId('dst');
                        i += 1;
                    }
                });
                ...
                elementIds.forEach(nextId => {
                    const nextNode = self._client.getNode(nextId);
                    if ((nextNode.isTypeOf(META['Inplaces']) || nextNode.isTypeOf(META['Outplaces'])) && nextNode.getPointerId('src') === elementId) {
                        transition.next[nextNode.getAttribute('name') + j] = nextNode.getPointerId('dst');
                        j += 1;
                    }
                });
                sm.places[elementId] = transition;
            }

        });

        elementIds.forEach(arc => {
            const a = self._client.getNode(arc);
            if (a.isTypeOf(META['Inplaces']) || a.isTypeOf(META['Outplaces'])) {
                const src = a.getPointerId('src');
                const dst = a.getPointerId('dst');
                if (dst in sm.transitions)
                    sm.transitions[dst].parents.push(src);
                if (src in sm.transitions)
                    sm.transitions[src].children.push(dst);
                if (dst in sm.places)
                    sm.places[dst].parents.push(src)
                if (src in sm.places)
                    sm.places[src].children.push(dst);
            }
        });
    ...
    PNVizControl.prototype.clearSM = function () {
        const self = this;
        self._networkRootLoaded = false;
        self._widget.destroyMachine();
    };

    PNVizControl.prototype.setFireableEvents = function (events) {
    ...
    PNVizControl.prototype.destroy = function () {
        this._detachClientEventListeners();
        this._removeToolbarItems();
    };

    PNVizControl.prototype._attachClientEventListeners = function () {
        this._detachClientEventListeners();
        WebGMEGlobal.State.on('change:' + CONSTANTS.STATE_ACTIVE_OBJECT, this._stateActiveObjectChanged, this);
    };

    PNVizControl.prototype._detachClientEventListeners = function () {
        WebGMEGlobal.State.off('change:' + CONSTANTS.STATE_ACTIVE_OBJECT, this._stateActiveObjectChanged);
    };

    PNVizControl.prototype.onActivate = function () {
    ...
    PNVizControl.prototype.onDeactivate = function () {
        this._detachClientEventListeners();
        this._hideToolbarItems();
    };

    /* * * * * * * * * * Updating the toolbar * * * * * * * * * */
    PNVizControl.prototype._displayToolbarItems = function () {
    ...
    PNVizControl.prototype._hideToolbarItems = function () {
    ...
    PNVizControl.prototype._removeToolbarItems = function () {
    ...
    PNVizControl.prototype._initializeToolbar = function () {
        var self = this,
        ...
        this.$btnReachCheck = toolBar.addButton({
            ...
            clickFn: function (/*data*/) {
                ...
                const context = self._client.getCurrentPluginContext('PNCodeGen', self._currentNodeId, []);
                console.log(context)
        ...
        this.$btnSingleEvent = toolBar.addButton({
            ...
            clickFn: function (/*data*/) {
                ...
                console.log("CLICK")
                console.log(self._fireableEvents[0]);
            }
        });
